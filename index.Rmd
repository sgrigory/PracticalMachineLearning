...
cM.df=as.data.frame((cM$table)/rowSums(cM$table))
cM.df$rgb=sapply(cM.df$Freq,function(x) rgb(x,0,0))
ggplot(data=cM.df)+geom_tile(aes(x=Prediction,y=Reference,fill=Freq))+scale_y_discrete(name="Actual Class")+scale_fill_gradient(breaks=seq(from=-.5, to=4, by=.2),low="white",high="red")
...
```{r, include=FALSE}
final.acc=sprintf("%.2f",mean(data.quiz$pred==correct))
```
